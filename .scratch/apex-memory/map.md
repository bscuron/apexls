...
- How workspace-wide LSP features (find-all-references, workspace symbol search, project-wide diagnostics/rename) should behave once most files are declaration-only instead of fully bound — depends on the working-set scoping design landing first. (Ticket 03 found these are the *only* real open question left for correctness — everything else about dependency closure is settled.)
- Stdlib reference-data representation tightening (interning/shrinking `apex_reference.json` / `standard_objects.json` in memory) — real but modest (~15-25MB); ticket 03's numbers suggest this isn't needed to hit the destination, so likely stays out of scope unless ticket 04 falls short.
- Salsa incremental-cache growth over a long-running session with many edits — the dhat profile only measured a single cold `from_files_cached` call, not steady-state after repeated incremental rebuilds. Worth investigating once a baseline architecture is chosen.
- `CandidateFile`/discovery-time allocation (~6% of peak in the profile) — unclear whether this is transient (freed after discovery) or retained; worth a quick look once the bigger structural decision is underway.
