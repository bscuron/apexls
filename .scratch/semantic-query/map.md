...
- **The escape hatch for what pattern literals cannot express.** Every tool in the survey ships
  ...
  `Script` filters, Coccinelle's `@script:python@` rules) because the pattern-literal ceiling is
  real. v1 ships none deliberately: building constraint syntax before a single real query has hit
  the ceiling means guessing which constraint is needed. Run the corpus against built search, and
  the queries that cannot be written will say whether it is `kind`, `regex`, both, or neither.
  Note that node-kind matching needs a `SyntaxKind::from_str` written first, and that ast-grep's
  own escape hatch (`context:` + `selector:`) costs the user grammar node names -- i.e. it leads
  straight back to "you must know the AST", which is the thing pattern literals exist to avoid.
