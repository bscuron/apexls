...
- ~~Constraints in the reserved `--where` slot~~ **-- resolved as `--and COND` / `--not COND`**
  ([match semantics](issues/02-match-semantics-decision.md), item 21). The pattern says the
  structure, `$X ~ GLOB` / `$X ~ /REGEX/` says a capture's text, and the two flags say yes or no;
  a condition that is not a `~` test is a pattern the match must contain. "These two captures
  differ" is `--not '$T ~ $V'`. No `--or` and no boolean operators inside a condition, by choice:
  alternation inside a glob or regex covers the common case. Inline globs were removed with it.
...
- ~~The escape hatch~~ **-- resolved, and the answer was "both".** Running the corpus against
  ...
  absence (since folded into `--and`/`--not` conditions). `SyntaxKind::from_name` is generated by the same macro as the enum, so it cannot
...
- ~~Binder-backed conditions~~ **-- the first one is built: `$X : TYPE`**
  ...
  open: *standard* child relationships (`account.Contacts`), which need relationship names the
  scraped standard-object data does not carry; static and instance initializer blocks (never
  bound); and resolving a captured type reference rather than using its text. What else can eventually go in it is bounded by the substrate note above: resolution and stdlib-membership
