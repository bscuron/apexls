...
//! all), so collection is embarrassingly parallel across files; see
//! `crate::BoundProgram::from_files` for where the per-file results get
//! merged (and their locally-numbered symbol indices remapped into the
//! shared `SymbolTable`'s global `SymbolId` space).
...
/// One file's collected declarations. `container` fields on `symbols`
/// are local indices into this same `Vec` (not yet global `SymbolId`s --
/// see the module doc comment) until merged into a shared `SymbolTable`.
#[derive(Default)]
pub(crate) struct FileCollection {
    pub(crate) symbols: Vec<Symbol>,
    /// `(local index of a type symbol in `symbols`, unresolved
    /// extends/implements supertype names as written)`, consumed by
    /// Pass 1.5 (`crate::inherit`) once every file's symbols have been
    /// merged into one global `SymbolTable`, to build the member-lookup
    /// `inherited_chain`.
    pub(crate) raw_extends: Vec<(u32, Vec<String>)>,
    /// `(local index of a *class* symbol, its direct `extends` target's
    /// unresolved name)`, present only when that class actually declared
    /// `extends` -- narrower than `raw_extends` (which also folds in
    ...
    pub(crate) raw_super: Vec<(u32, String)>,
}
...
fn local(idx: Option<u32>) -> Option<SymbolId> {
    idx.map(SymbolId)
}

fn type_ptr_and_name(ty: Option<Type>) -> (Option<AstPtr<Type>>, Option<String>) {
    match ty {
...
pub(crate) fn collect_trigger_unit(file: FileId, tu: &TriggerUnit) -> FileCollection {
    ...
    let trigger_id = out.push(Symbol {
        kind: SymbolKind::Trigger,
        name: name_text,
        file,
        ptr: SyntaxPtr::new(tu.syntax()),
...
fn collect_class(
    ...
    let class_id = out.push(Symbol {
        kind: SymbolKind::Class,
        name: name_text,
        file,
        ptr: SyntaxPtr::new(class.syntax()),
        name_range: name.syntax().text_range(),
...
fn collect_interface(
    ...
    let iface_id = out.push(Symbol {
        kind: SymbolKind::Interface,
        name: name_text,
        file,
        ptr: SyntaxPtr::new(iface.syntax()),
        name_range: name.syntax().text_range(),
        container: local(container),
        type_ref: None,
        type_name: None,
...
fn collect_enum(out: &mut FileCollection, file: FileId, en: &EnumDecl, container: Option<u32>) {
    ...
    let enum_id = out.push(Symbol {
        kind: SymbolKind::Enum,
        name: name_text,
        file,
        ptr: SyntaxPtr::new(en.syntax()),
        name_range: name.syntax().text_range(),
        container: local(container),
        type_ref: None,
        type_name: None,
    ...
    if let Some(constants) = en.constant_list() {
        for constant in constants.constants() {
            ...
            out.push(Symbol {
                ...
                name: constant_text,
                file,
                ptr: SyntaxPtr::new(constant.syntax()),
                name_range: constant.syntax().text_range(),
                container: local(Some(enum_id)),
                type_ref: None,
                type_name: None,
                modifiers: ModifierSet::default(),
            });
...
fn collect_member(out: &mut FileCollection, file: FileId, member: &Member, container: u32) {
    match member {
...
fn collect_method(out: &mut FileCollection, file: FileId, m: &MethodDecl, container: u32) {
    ...
    let (type_ref, type_name) = type_ptr_and_name(m.return_type());
    let method_id = out.push(Symbol {
        kind: SymbolKind::Method,
        name: name_text,
        file,
        ptr: SyntaxPtr::new(m.syntax()),
        name_range: name.syntax().text_range(),
        container: local(Some(container)),
        type_ref,
        type_name,
...
fn collect_constructor(
    ...
    let ctor_id = out.push(Symbol {
        kind: SymbolKind::Constructor,
        name: type_ref.text(),
        file,
        ptr: SyntaxPtr::new(c.syntax()),
...
fn collect_field(out: &mut FileCollection, file: FileId, f: &FieldDecl, container: u32) {
    let (type_ref, type_name) = type_ptr_and_name(f.type_ref());
    for declarator in f.declarators() {
        ...
        out.push(Symbol {
            ...
            name: name_text,
            file,
            ptr: SyntaxPtr::new(declarator.syntax()),
            name_range: name.syntax().text_range(),
            container: local(Some(container)),
            type_ref,
...
fn collect_property(out: &mut FileCollection, file: FileId, p: &PropertyDecl, container: u32) {
    ...
    let (type_ref, type_name) = type_ptr_and_name(p.type_ref());
    out.push(Symbol {
        kind: SymbolKind::Property,
        name: name_text,
        file,
        ptr: SyntaxPtr::new(p.syntax()),
        name_range: name.syntax().text_range(),
        container: local(Some(container)),
        type_ref,
        type_name,
...
fn collect_params(
    ...
    for param in params.params() {
        ...
        let (type_ref, type_name) = type_ptr_and_name(param.type_ref());
        out.push(Symbol {
            kind: SymbolKind::Parameter,
            name: name_text,
            file,
            ptr: SyntaxPtr::new(param.syntax()),
            name_range: name.syntax().text_range(),
            container: local(Some(container)),
            type_ref,
            type_name,
