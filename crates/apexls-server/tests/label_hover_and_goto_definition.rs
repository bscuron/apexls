//! `textDocument/hover` and `textDocument/definition` on a
//! `Resolution::Label` reference (`Label.<name>`/`System.Label.<name>`) --
//! confirms the custom-label integration (`.labels-meta.xml` discovery,
//! `apex_metadata::LabelSchema`, `apex_binder::LabelIndex`) reaches real
//! LSP responses, not just the underlying `Resolution` outcome
//! `label_resolution.rs` (in `apex-binder`) already covers. Follows
//! `schema_goto_definition.rs`'s exact protocol-level harness pattern
//! (spawn the real binary, drive it over real stdio, wait for "rebuild
...
#[test]
fn hover_on_a_label_reference_shows_its_declared_value() {
    let src = "public class Foo {\n    public String run() {\n        return System.Label.greeting;\n    }\n}\n";
    ...
    let (line, character) = position_of(src, "greeting");
    ...
    assert!(
        contents.contains("Hello there"),
        "hover text should show the label's declared value: {contents}"
    );
...
#[test]
fn definition_on_a_label_reference_points_at_its_labels_meta_xml() {
    ...
    let foo_uri = Url::from_file_path(dir.join("Foo.cls")).unwrap();
    let labels_meta_uri =
        Url::from_file_path(dir.join("labels/CustomLabels.labels-meta.xml")).unwrap();
    ...
    assert!(response.get("error").is_none(), "definition returned an error: {response:?}");
    let result = &response["result"];
    let uri = result["uri"]
        .as_str()
        .unwrap_or_else(|| panic!("expected a scalar Location, got {response:?}"));
    assert_eq!(uri, labels_meta_uri.as_str());
