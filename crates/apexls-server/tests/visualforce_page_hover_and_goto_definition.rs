...
//! `Resolution::VisualforcePage` reference (`Page.<name>`) -- confirms the
//! Visualforce page integration (`apex_discover::Discovery::page_files`,
//! `apex_binder::PageIndex`) reaches real LSP responses, not just the
//! underlying `Resolution` outcome `visualforce_page_resolution.rs` (in
//! `apex-binder`) already covers. Follows `label_hover_and_goto_definition.rs`'s
//! exact protocol-level harness pattern.
...
const MY_PAGE_MARKUP: &str = "<apex:page>Hello</apex:page>";

#[test]
fn hover_on_a_visualforce_page_reference_shows_it_resolved() {
    let src = "public class Foo {\n    public PageReference run() {\n        return Page.MyPage;\n    }\n}\n";
    let dir = write_fixture_dir(
        "vf-page-hover",
        &[("Foo.cls", src), ("pages/MyPage.page", MY_PAGE_MARKUP)],
    );
    ...
    let (line, character) = position_of(src, "MyPage");
    ...
    assert!(
        contents.contains("MyPage"),
        "hover text should mention the page's own name: {contents}"
    );
...
#[test]
fn definition_on_a_visualforce_page_reference_points_at_its_page_file() {
    let src = "public class Foo {\n    public PageReference run() {\n        return Page.MyPage;\n    }\n}\n";
    let dir = write_fixture_dir(
        "vf-page-goto-def",
        &[("Foo.cls", src), ("pages/MyPage.page", MY_PAGE_MARKUP)],
    ...
    let root_uri = Url::from_file_path(&dir).unwrap();
    let foo_uri = Url::from_file_path(dir.join("Foo.cls")).unwrap();
    let page_uri = Url::from_file_path(dir.join("pages/MyPage.page")).unwrap();
